...
func processDirectory(dirname string) {
	...
	var gic *Checker
	if options.Git {
		gic = NewCheckerWithCache(global.gitignoreCache)
	...
	nextEntry:
		for _, fi := range entries {
			...
			if fi.IsDir() {
				...
				if options.Git {
					if fi.Name() == GitFoldername || gic.Check(fullpath, fi) {
						continue nextEntry
					}
				}
			...
			if options.Git {
				if fi.Name() == GitIgnoreFilename || gic.Check(fullpath, fi) {
					continue
				}
			}
...
func processFileTargets() {
	...
	for filepath := range global.filesChan {
		...
		if options.Zip && strings.HasSuffix(filepath, ".gz") {
			...
			reader = NewNBReader(infile, InputBlockSize,
				ChunkTimeout(MultilinePipeChunkTimeout), Timeout(MultilinePipeTimeout))
		} else {
			reader = infile
		}
...
func processNetworkTarget(target string) {
	...
	if options.Multiline {
		reader = NewNBReader(conn, InputBlockSize, ChunkTimeout(MultilinePipeChunkTimeout),
			Timeout(MultilinePipeTimeout))
	} else {
		reader = conn
	}
...
func executeSearch(targets []string) (ret int, err error) {
	...
	global.gitignoreCache = NewGitIgnoreCache()
